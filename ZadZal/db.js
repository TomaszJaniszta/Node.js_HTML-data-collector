...
const updateAd = (id, modifiedAd) => {
    return adsCollection.updateOne(
        { _id: new ObjectId(id)},
        { $set: { "title": (modifiedAd.title), "description": (modifiedAd.description),
                    "author": (modifiedAd.author), "category": (modifiedAd.category),
                    "tags": (modifiedAd.tags), "price": (modifiedAd.price), "location": (modifiedAd.location) }}
    );
}
